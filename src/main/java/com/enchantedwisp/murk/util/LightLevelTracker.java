...
public class LightLevelTracker {
    ...
    public static void register() {
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            int ticksUntilWarning = (int) (config.general_warningMessageDelay * 20); // Convert seconds to ticks
            int ticksAfterWarning = (int) (config.general_effectDelayAfterWarning * 20); // Convert seconds to ticks
            ...
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                ...
                if (lightLevel < lightThreshold) {
                    ...
                    if (!player.hasStatusEffect(Effects.MURKS_GRASP)) {
                        ...
                        // Send warning message to action bar after configured delay if enabled
                        if (enableWarningText && ticks >= ticksUntilWarning && !playerWarned.getOrDefault(playerId, false)) {
